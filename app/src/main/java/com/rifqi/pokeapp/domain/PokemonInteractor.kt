...
class PokemonInteractor(private val iPokemonRepository: IPokemonRepository) : PokemonUseCase {
    override fun getListPokemon(url : String): Flow<Resource<List<ItemPokemon>>> = iPokemonRepository.getListPokemon(url)
    override fun getDetailPokemon(url: String): Flow<Resource<GetDetailPokemon>> = iPokemonRepository.getDetailPokemon(url)
}